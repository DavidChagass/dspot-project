...
<div class="flex bg-white py-24 sm:py-32">
    <div class="mx-auto max-w-7xl gap-20 px-6 lg:px-8">
        <form class="flex flex-col" action="{{ route('empresa.estoque.store') }}" method="POST">
            @csrf
            <div class="mb-2">
                <label class="block pr-2 text-sm font-medium leading-6 text-gray-900" for="nome">nome do estoque</label>
                <input class="bg-blue-100 block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6" type="text" name="nome" required id="nome" wire:model="nome">
                @error('nome')
            ...
            <div class="flex m-2 justify-around">
                ...
                <button type="submit" class="rounded-md bg-gray-900 px-3 py-2 text-sm font-medium text-white">salvar</button>
            </div>
        </form>
    </div>
</div>